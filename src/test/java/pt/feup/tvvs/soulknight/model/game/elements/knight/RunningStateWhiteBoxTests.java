...
public class RunningStateWhiteBoxTests {
    Knight knight;
    Scene scene;
    RunningState state;
    ...
    void setup() {
        ...
        state = spy(new RunningState(knight));
    ...
    @Test
    public void testGetNextStateMaxingVelocityState() {
        when(scene.collideSpike()).thenReturn(false);
        when(state.getParticlesTimer()).thenReturn(5L);
        when(knight.isOverMaxXVelocity()).thenReturn(false);
        when(knight.isOnGround()).thenReturn(true);
        when(knight.getVelocity()).thenReturn(new Vector(RunningState.MAX_VELOCITY + 1, 0));

        assertTrue(state.getNextState() instanceof MaxVelocityState);
    }

    @Test
    public void testGetNextStateWalkingState() {
        ...
        assertTrue(state.getNextState() instanceof WalkingState);
    }

    @Test
    public void testGetNextStateStayRunning() {
        when(knight.getVelocity()).thenReturn(new Vector(1, 1));
        when(scene.collideSpike()).thenReturn(false);
        when(state.getParticlesTimer()).thenReturn(5L);
        when(knight.isOverMaxXVelocity()).thenReturn(false);
        when(knight.isOnGround()).thenReturn(true);
        when(knight.getVelocity()).thenReturn(new Vector(RunningState.MIN_VELOCITY, 0));
